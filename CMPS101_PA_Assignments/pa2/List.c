...
void deleteFront(List myList)
{
    ...
    {
        if(myList->cursor == myList->front || myList->cursor == myList->back)
        {
            myList->cursor = NULL;
            myList->index = -1;
        }
...
void delete(List myList)
{
 if(myList->length == 0)
    ...
    if(myList->index == 0 && ((myList->length)-1))
    {
        myList->front = myList->front->next;
        myList->length = ((myList->length)-1);
        myList->cursor = NULL;
    ...
    else if(myList->index != 0 && myList->index != ((myList->length)-1))
    {
    ...
    else if(myList->index == ((myList->length)-1) && (myList->length) != 1)
    {
        myList->back = myList->back->prev;
        myList->back->next = NULL;
        myList->length = ((myList->length)-1);
