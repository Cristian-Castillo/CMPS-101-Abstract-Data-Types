...
// Spring 2019 
// Assignment PA3 Note: Moded ADT List PA1 List.java
...
{   // class node will hold data
    ...
    {
        Object data;                       //inner node initialized integer variable data
        Node next;
        Node prev;
 ...
 // Access functions ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    // are not used in determining equality.
     public boolean equals(Object x)      // passed List an instiante blank object
     {
   ...
   // Insert new element before cursor.
    ...
    // Pre: length()>0, index()>=0
     // if there is nothing in the list, then we cant delete anything!
    // if our index is arr[0] position and not at the end then we are at the head
    // and since we are at position arr[0], then we reassign the head to the 
    // second node as the new head, and decrement the list size
    // and our cursor never moved, and thus becomes undefined
    // if we arent at the beginning of our list Node (arr[0]) and not at the end arr[n+1]
    // we are tying cursor position cursor=cursor+1 with cursor pos at cursor= cursor -1
    // so envision | 1 |-> | 2 | -> | 3 |, and 3 is being tied to 1
    // and 1 is now pointing to 3, and thus node | 2 | is now being deleted
    // and since the node was deleted, then our index becomes undefined
    // and evidently the loss of a node, means to decrement the list
    // else if the index is position at a[n+1]; end of list and not at the beginning
    // our tail will be reassigned to the second to last node on the list
    // and this node will now cut off. the last node by setting tail.next to the End of list
    // losing a node indicates that we decrement the size of the list
    // since our cursor was position at the end, and never moved; our cursor becomes undefined with the index
    void delete(){
    ...
    List copy()
    {
        ...
        if(length() == 0)
        {
            return(list);                                // if no list exist then return the empty list with its defined features
        }
