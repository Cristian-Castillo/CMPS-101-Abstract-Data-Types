/*
 ...
 Assignment: PA4-List.h
 Date: May 21, 2019
 Name: Cristian C. Castillo Jr
 Cruz ID: ccarri11
 Student ID: #1576341
 Note: Preserved pa2, with adjustments to memory leaks
 */
// ----------------------------------------------------------------------------------------------------------
//Header File
#ifndef List_H
...
typedef struct ListObj* List; // "typedef" allows you to make your own 'type' in plain C.  This is being exported to the main (lex.c) because Lex.c includes List.h, but not List.c
							  // You now have a struct pointer of "ListObj" type that is called "List", and we know that List has two attributes - front and back reference or aka head/tail, 
...
struct NodeObj;				  // an "int data" attrubute", a "next" attribute, which is a recursively defined NodeObject type itself, and a "previous" attribute which is also a recursively defined
							  // NodeObect type itself - and so thse two attributes, (next and prev) also have three attributes themselves, int data, next, and prev, and so on.
