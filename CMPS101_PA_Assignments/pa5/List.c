...
void deleteBack(List L)
{
    ...
    if(L->length < 1)
    {
        printf("Invoked List(); Referenced empty list, no element detected.\n");   
        exit(1);
    }
    if(L->length == 1)
    {
        
        if(L->cursor == L->back || L->cursor == L->front)
        {
...
/*
 ...
 {
 Node nodePtr = head;                             // set our garbage nodePtr to the front of the list
